<?php
	...
	header("Location: charging_clients_per_client_detail.php?date_from=$date_from&date_to=$date_to&client_code=$client_code");
	}
	
?>
		<h1><a href='<?php echo "charging_clients.php?date_from=$date_from&date_to=$date_to"; ?>' style="text-decoration:none;" title="Back">&#8656;</a> Client Charging per Client</h1>
...
<?php
		...
		$query = $mysqli->query("SELECT logs.log_employee_id_no, employees.employee_id_no, employees.employee_last_name, logs.log_client_code FROM employees INNER JOIN logs ON employees.employee_id_no = logs.log_employee_id_no WHERE logs.log_date BETWEEN '$date_from' AND '$date_to' AND logs.log_client_code = '$client_code' GROUP BY logs.log_employee_id_no ORDER BY employees.employee_last_name");
		$a = 1;
