<?php 
	...
	$log_position_code = employee_position($employee_data['employee_id']);
	$log_period_code = period_active_last();
	...
	if (empty($_POST) === false) { 
		...
		if (isset($_POST['insert'])) { 
			...
			if (empty($errors)) {
				...
				log_a, log_b, log_c, log_d, log_e, log_f, log_g, log_h, log_description) VALUES ('$log_employee_id_no', '$log_position_code', '$log_period_code', '" . date("Y-m-d", strtotime($log_date)) . "', $log_in, 
				$log_out, $log_client_code, $log_regular, $log_late, log_undertime,	$log_a, $log_b, $log_c, $log_d, $log_e, $log_f, $log_g, $log_h, $log_description)");
				include 'core/database/close.php' ;
...
?>
	<h1><a href="logs_list.php" style="text-decoration:none;" title="Back">&#8656;</a> Insert Log</h1>
...
?>
		...
		<input type="hidden" name="log_employee_id_no" value="<?php echo $employee_data['employee_id_no']; ?>" class="text" /></td>	
...
?>
		...
		<label for="log_a" class="span2">Overtime<i> (hour)</i></label>
			...
			<sup>h</sup>
			<?php /*<input type="number" step="any" name="log_f" class="span0" id="log_f" placeholder="F" value="<?php if (isset($log_f)) { echo $log_f; } ?>" maxlength="5" onkeyup="float_numbers(this)" oninput="log_overtime_total()" />
			<sup>f</sup>*/?>			
		<br /><br />
