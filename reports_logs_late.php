	...
	<table class="table_list print">					
		<thead>
			...
			<tr class="bg-color-dimgrey fg-color-white">						
				...
				<th class="center" style="min-width:50px;">Total Cost per Employee</th>
...
<?php
		...
		$total_late_per_emp = 0;
		while($rows1 = $query1->fetch_assoc()) {
			$employee_id_no1 = $rows1['employee_id_no'];	
			$query = $mysqli->query("SELECT		SUM(log_late) as log_late,
												logs.log_id,
												logs.log_employee_id_no,
												logs.log_position_code,
			...
			while($rows = $query->fetch_assoc()) {
				...
				if($rows) { // get data from db
					...
					$log_late_min = $log_late / 60;
					$late_cost =  number_format($position_rate, 2, '.', ',') * number_format($log_late_min, 2, '.', ',');
					$total_late_cost_per_emp = $late_cost;
					$sum_log_late_per_emp = $log_late;
				...
				$overall_total_late = $overall_total_late + $total_late_cost_per_emp;
...
?>
			<tr class="alternate">
				...
				<td class="right"><?php echo (!empty($log_late)) ?  number_format($log_late, 2, '.', ',') : "-"; ?></td>
				<td class="right"><?php echo (!empty($log_late)) ?  number_format($total_late_cost_per_emp, 2, '.', ',') : "-"; ?></td>
...
?>
			<tr class="bg-color-lightblue">
				...
				<td class="right bold"><?php echo (!empty($total_late)) ?  number_format($overall_total_late, 2, '.', ',') : "-"; ?></td>
