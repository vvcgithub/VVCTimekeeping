...
<?php
		if($current_page!=1)
		{
			...
			echo "<li><a href='?page=1&from=$from&to=$to'>First</a></li>";
			$previous = $current_page-1;
			echo "<li><a href='?page=$previous&from=$from&to=$to'>Previous</a></li>";
		}
		...
		for($page=$beginno;$page<=$endno;$page++) {
			echo ($page == $current_page) ? "<li class='bold'><a href='?page=$page&from=$from&to=$to'>$page</a></li>" : "<li><a href='?page=$page&from=$from&to=$to'>$page</a></li>";
		}
		if($current_page < $numpages) {
			$next=$current_page+1;
			echo "<li><a href='?page=$next&from=$from&to=$to'>Next</a></li>";
			echo "<li><a href='?page=$numpages&from=$from&to=$to'>Last</a></li>";
		}
?>
	</ul>
</div>