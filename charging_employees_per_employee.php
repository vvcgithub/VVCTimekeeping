...
?>
		<h1><a href='<?php echo "charging_employees.php?date_from=$date_from&date_to=$date_to"; ?>' style="text-decoration:none;" title="Back">&#8656;</a> Employees Charging per Employee</h1>
...
<?php
		...
		$query = $mysqli->query("SELECT DISTINCT logs.log_client_code, logs.log_employee_id_no, clients.client_id, clients.client_code, clients.client_name FROM clients INNER JOIN logs ON clients.client_code = logs.log_client_code WHERE logs.log_date BETWEEN '$date_from' AND '$date_to' AND logs.log_employee_id_no = '$employee_id_no' ORDER BY clients.client_code");
		$a = 1;
