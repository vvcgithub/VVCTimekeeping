...
?>
	<h1><a href="charging_main.php#charging_clients_parameter" style="text-decoration:none;" title="Back">&#8656;</a> Client Charging</h1>
...
<?php
			...
			$query = $mysqli->query("SELECT DISTINCT logs.log_client_code, clients.client_id, clients.client_code, clients.client_name FROM clients INNER JOIN logs ON clients.client_code = logs.log_client_code WHERE logs.log_date BETWEEN '$date_from' AND '$date_to' ORDER BY clients.client_code");
			$a = 1;
			while($rows = $query->fetch_assoc()) {
				$client_code = $rows['client_code'];	
				$client_name = $rows['client_name'];	
				$query_log = $mysqli->query("SELECT SUM(log_regular) as log_regular_sum, SUM(log_a) as log_a_sum, SUM(log_b) as log_b_sum, SUM(log_c) as log_c_sum, SUM(log_d) as log_d_sum, SUM(log_e) as log_e_sum, SUM(log_f) as log_f_sum FROM logs WHERE log_client_code = '$client_code' AND log_date BETWEEN '$date_from' AND '$date_to'");
...
?>
				<tr class="alternate">
					...
					<td><a href="charging_clients_per_client.php?date_from=<?php echo $date_from; ?>&date_to=<?php echo $date_to; ?>&client_code=<?php echo $client_code; ?>"><?php echo $client_name; ?></a></td>
