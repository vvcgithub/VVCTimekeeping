<?php 
	...
	if (isset($_GET['log_id'])) {
		...
		$query = $mysqli->query("SELECT * FROM logs WHERE log_id = $log_id AND log_employee_id_no = $log_employee_id_no") or die(mysql_error()); 
		...
		if($rows) {
			...
			header("Location: logs_list.php?page=$page");
			exit();
		}
		include 'core/database/connect.php' ;
	}
	...
	if (isset($_POST['fieldwork']) || isset($_POST['biometrics']) || isset($_POST['regular']) || isset($_POST['update']) || isset($_POST['late']) || isset($_POST['undertime'])) { 
		...
		if (isset($_POST['update'])) { 
			...
			if (empty($errors)) {
				$log_date = (empty($log_date))? 'NULL' : "'". date("Y-m-d", strtotime($log_date)) . "'";
...
?>
	<h1><a href="logs_list.php" style="text-decoration:none;" title="Back">&#8656;</a> Update Log</h1>
...
?>
		...
		<input type="hidden" name="log_employee_id_no" value="<?php echo $employee_data['employee_id_no']; ?>"  />
...
?>
		...
		<input type="date" id="date" name="log_date" maxlength="10" class="span2" style="font-family:arial;" value="<?php echo (isset($log_date) && $log_date <> NULL) ? date("Y-m-d", strtotime($log_date)) : "" ; ?>" placeholder="yyyy-mm-dd" required="required" />
		<br /><br />
...
?>
		...
		<label for="log_a" class="span2">Overtime<i> (hour)</i></label>
			...
			<sup>c</sup>	
			<input type="number" step="any" name="log_d" class="span0" id="log_d" placeholder="D" value="<?php if (isset($log_d)) { echo $log_d; } ?>" maxlength="5" onkeyup="float_numbers(this)" oninput="log_overtime_total()" />
			<sup>d</sup>		
			...
			<sup>e</sup>
			<input type="number" step="any" name="log_f" class="span0" id="log_f" placeholder="F" value="<?php if (isset($log_f)) { echo $log_f; } ?>" maxlength="5" onkeyup="float_numbers(this)" oninput="log_overtime_total()" />
			<sup>f</sup>
			<input type="number" step="any" name="log_g" class="span0" id="log_g" placeholder="G" value="<?php if (isset($log_g)) { echo $log_g; } ?>" maxlength="5" onkeyup="float_numbers(this)" oninput="log_overtime_total()" />
			<sup>g</sup>
			<input type="number" step="any" name="log_h" class="span0" id="log_h" placeholder="H" value="<?php if (isset($log_h)) { echo $log_h; } ?>" maxlength="5" onkeyup="float_numbers(this)" oninput="log_overtime_total()" />
			<sup>h</sup>
			<input type="number" step="any" name="log_i" class="span0" id="log_i" placeholder="I" value="<?php if (isset($log_i)) { echo $log_i; } ?>" maxlength="5" onkeyup="float_numbers(this)" oninput="log_overtime_total()" />
			<sup>i</sup>			
		<br /><br />
