<?php
	...
	if (isset($_POST['back'])) {
		header("Location: charging_clients_per_client.php?date_from=$date_from&date_to=$date_to&client_code=$client_code");
		exit();
	}
?>
		<h1><a href='<?php echo "charging_clients_per_client.php?date_from=$date_from&date_to=$date_to&client_code=$client_code"; ?>' style="text-decoration:none;" title="Back">&#8656;</a> Client Charging per Client</h1>
	...
	<table class="print">					
		<thead>
			...
			<tr class="bg-color-dimgrey fg-color-white">						
				...
				<th class="center" style="min-width:350px;">Description</th>
			</tr>
		</thead>
		<tbody>			
<?php
		include 'core/database/connect.php' ;
		$query1 = $mysqli->query("SELECT DISTINCT logs.log_employee_id_no, employees.employee_id_no, employees.employee_last_name, logs.log_client_code FROM employees INNER JOIN logs ON employees.employee_id_no = logs.log_employee_id_no WHERE logs.log_date BETWEEN '$date_from' AND '$date_to' AND logs.log_client_code = '$client_code' ORDER BY employees.employee_last_name");
		$grand_total =0;
		while($rows1 = $query1->fetch_assoc()) {
			$employee_id_no1 = $rows1['employee_id_no'];	
			$query = $mysqli->query("SELECT logs.log_id, logs.log_employee_id_no, employees.employee_id_no, employees.employee_last_name, logs.log_client_code FROM employees INNER JOIN logs ON employees.employee_id_no = logs.log_employee_id_no WHERE logs.log_employee_id_no = '$employee_id_no1' AND logs.log_date BETWEEN '$date_from' AND '$date_to' AND logs.log_client_code = '$client_code' ORDER BY logs.log_date");
			$a = 1;
			$total_cost = 0;
			while($rows = $query->fetch_assoc()) {
				...
				$query_log = $mysqli->query("SELECT log_position_code, log_regular, log_date, log_a, log_b, log_c, log_d, log_e, log_f, log_description FROM logs WHERE log_id = '$log_id'");
...
?>
				<tr>
					...
					<td class="left"><?php echo (!empty($log_description)) ? $log_description : ""; ?></td>
				</tr>					
<?php
				$a = $a + 1;
