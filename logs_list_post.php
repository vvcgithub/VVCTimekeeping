<?php 
	...
	if(isset($_GET['page'])) {
		...
		$current_page = 1;
		header('Location: logs_post_list.php?page=1');
	}
	...
	if (isset($_POST['refresh'])) {
		header('Location: logs_list_post.php?page=1');
		exit();
	}
	
	if (isset($_POST['add'])) {
		header('Location: logs_post_insert.php');
		exit();
	}
	
	if (isset($_POST['print'])) {
		header('Location: logs_post_print_parameter.php');
		exit();
	}
	
	if (isset($_POST['template'])) {
		header('Location: logs_post_template.php');
		exit();
	}
	
	if (isset($_GET['upload'])) {
		header('Location: logs_post_uploader.php');
		exit();
	}
	
	if (isset($_GET['download'])) {
		$employee_id = $employee_data['employee_id'];
		header("Location: logs_post_report_to_csv_final.php?employee_id=$employee_id&date_from_final=&date_to_final=");
		exit();
	}
	
	if (isset($_GET['message'])) {
		header('Location: logs_post_message.php');
		exit();
	}
	...
	if (isset($_POST['import'])) {
		header('Location: logs_post_import.php');
		exit(); 
	}
	
	if (isset($_POST['delete_all'])) {
		header('Location: logs_post_list_delete.php');
		exit(); 
	}
	...
	$query = $mysqli->query("SELECT * FROM logs_post ORDER BY log_post_id ASC");
	...
	if (isset($_POST['delete_selected'])) {
		...
		if (isset($_POST['unique_id']) && !empty($_POST['unique_id'])) {
			...
			foreach ($unique_id as $id => $value) {
				$mysqli->query("DELETE FROM logs_post WHERE log_post_id = '$value' AND log_check = 0");
			}
			header("Location: logs_post_list.php?page=$current_page");
			exit();
		}
		include 'core/database/close.php' ;
	}
	...
	if(isset($_GET['delete'])) { 
		$log_post_id = $_GET['delete'];
		include 'core/database/connect.php' ;
		$mysqli->query("DELETE FROM logs_post WHERE log_post_id='$log_post_id'");
		include 'core/database/close.php' ;
		header('Location: logs_post_list.php?page=' . $current_page);
		exit();
	}
...
<?php	
				include 'core/database/connect.php' ;
				$query = $mysqli->query("SELECT log_post_id, log_employee_id_no, log_period_code, log_date, log_in, log_out, log_client_code, log_regular, log_late, log_undertime, log_a, log_b, log_c, log_d, log_e, log_f, log_description, log_check, log_check FROM logs_post ORDER BY log_post_id ASC, log_date DESC LIMIT $limit ,$per_page");
...
?>				
					<tr id="<?php echo $log_post_id; ?>" name="row_id[]" style="<?php echo ($log_check == 1) ? 'background:lightgreen; color:grey;' : ''; ?>" title="<?php echo $log_description; ?>" class="alternate">
						<td class="center">
<?php
							if ($log_check == 0) {
?>
							<a href="logs_post_update.php?log_post_id=<?php echo $log_post_id ; ?>&page=<?php echo $current_page ; ?>">Edit</a>
...
?>
								<a href="#" onclick="delete_confirm_for_logs_post('<?php echo $current_page; ?>', '<?php echo $log_post_id; ?>', '<?php echo $log_date. ", " . $log_weekday; ?>', '<?php echo $log_in; ?>', '<?php echo $log_out; ?>', '<?php echo $log_client_code; ?>')">Delete</a>
<?php								
...
?>
						...
						<td class="center"><?php echo $log_post_id; ?></td>
