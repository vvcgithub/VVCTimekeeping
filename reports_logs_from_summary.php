<?php
	...
	$query_log = mysqli_query($mysqli, "SELECT * FROM logs WHERE log_employee_id_no = '$employee_id_no' AND log_period_code = '$period_code' ORDER BY log_date ASC");
	$row_sum = mysqli_fetch_array(mysqli_query($mysqli, "SELECT SUM(log_late) as log_late_sum, SUM(log_undertime) as log_undertime_sum, SUM(log_a) as log_a_sum, SUM(log_b) as log_b_sum, SUM(log_c) as log_c_sum, SUM(log_d) as log_d_sum, SUM(log_e) as log_e_sum, SUM(log_f) as log_f_sum, SUM(log_regular) as log_regular_sum FROM logs WHERE log_employee_id_no='$employee_id_no' AND log_period_code = '$period_code' ORDER BY log_date ASC"));
	...
	if (isset($_POST['close'])) {
		header("Location: reports_summary.php?page=1&period_code=$period_code&number_page=30");
		exit();
	}
...
<?php
							...
							$loa_sl = (isset($log_period_code) && isset($log_employee_id_no)) ? loa_sl($log_period_code, $log_employee_id_no) : 0;
							$loa_vl = (isset($log_period_code) && isset($log_employee_id_no)) ? loa_vl($log_period_code, $log_employee_id_no) : 0;
							$loa_lwop = (isset($log_period_code) && isset($log_employee_id_no)) ? loa_lwop($log_period_code, $log_employee_id_no) : 0;
